        ...
        if not arcs:
            ...
            while (not quene.empty()):
                ...
                if Y in assignment.keys():
                    x, y = self.crossword.overlaps[X, Y]
                    for word in self.domains[X]:
                        if len(word) - 1 >= x and word[x] != assignment[Y][y]:
                            removeFromX.append(word)
                            changed = True
                if changed:
                    for neighbor in self.crossword.neighbors(X):
                        quene.add((neighbor, X))
                    for word in removeFromX:
                        self.domains[X].remove(word)
                        if X in removeddomain.keys():
                            removeddomain[X] += word
                        else:
                            removeddomain[X] = word
